...
# BMADTest - Epic Breakdown

## Overview

This document provides the complete epic and story breakdown for BMADTest, decomposing the requirements from the PRD, UX Design if it exists, and Architecture requirements into implementable stories.
...
- Architecture: Nx Monorepo (Angular + FastAPI)
...
- Project Context: Strict `camelCase` for all API JSON payloads; `snake_case` for Python and DB.
...
**Given** The developer has Node.js, Python 3.11, and Docker installed
**When** They run the initialization command defined in the Architecture
**Then** A new Nx workspace is created with `apps/web-client` (Angular) and `apps/api-server` (FastAPI)
**And** The `docker-compose.yml` successfully spins up PostgreSQL 18.1
**And** `nx serve` starts both frontend and backend servers successfully
...
**And** Pydantic v2 models validate the login request payload
...
**Then** SQLAlchemy creates the `pat_patients` table
**And** The table includes columns for `mrn` (unique, indexed), `first_name`, `last_name`, `dob`, and `gender`
**And** Pydantic models exist for `PatientCreate` and `PatientResponse`
